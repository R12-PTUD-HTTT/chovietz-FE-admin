...
import { getShipperApplyForm, createShipperAccount } from "../../../../api/adminApi";
...
function ManageShipperAccount({}) {
    ...
    useEffect(async () => {
        try {
            setLoading(true);
            const data = await getShipperApplyForm();
            if (data.status === 200) {
    ...
    const handleClick = async (e) => {
        ...
        try {
            const result = await createShipperAccount(e.target.id);
                                        ...
                                        {applyList && applyList.length > 0 && applyList.map((item,key) => {
                                            ...
                                            return (
                                                <tr key={key}>
                                                    ...
                                                    <td>
                                                        <p className=" font-weight-bold mb-0">{item.gender}</p>
                                                    </td>
                                                    <td>
                                                        <p className=" font-weight-bold mb-0">{item.email}</p>
                                                    </td>
                                                    <td>
                                                        <p className=" font-weight-bold mb-0">{item.tel}</p>
                                                    </td>
                                                    <td className="align-middle text-center ">
                                                        <span className="text-secondary font-weight-bold">{item.address}</span>
                                                    </td>
                                                    <td className="align-middle text-center">
                                                        <span className="text-secondary font-weight-bold">{item.Work_area}</span>
                                                    </td>
